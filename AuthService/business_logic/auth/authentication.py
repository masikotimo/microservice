...
class FleetManagerEmailAndPasswordAuthentication(UserAuthentication):
    ...
    def login(self, login_data):
        try:
            ...
            if fleetmanager_count < 1:
                raise AuthenticationFailed(
                    'Invalid Credentials, Possibly you have not yet signed up as a Client.')
            fleetmanager = FleetManagerManager(
            ).get_list().filter(user=user)[0]
            return_object = {
                'login_status': 'success',
                'email': user.email,
                'user_id': user.Id,
            ...
            return_object['fleetmanager_id'] = fleetmanager.id
            user.last_login = timezone.now()
            user.save()
            return return_object
        except Exception as exception:
...
class SystemAdminEmailAndPasswordAuthentication(UserAuthentication):
    ...
    def login(self, login_data):
        try:
            ...
            if systemadmin_count < 1:
                raise AuthenticationFailed(
                    'Invalid Credentials, Possibly you are not registered as a Courier.')
            systemadmin = SystemAdminManager().get_list().filter(user=user)[0]
            return_object = {
                'login_status': 'success',
                'email': user.email,
                'user_id': user.Id,
            ...
            return_object['systemadmin_id'] = systemadmin.id
            user.last_login = timezone.now()
            user.save()
            return return_object
        except Exception as exception:
